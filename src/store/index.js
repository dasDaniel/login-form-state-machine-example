...
export const store = new Vuex.Store({
  ...
  getters: {
    ...
    login_showPasswordReset: state => {
      return [
        LOGIN_STATE.PASSWORD_RESET,
        LOGIN_STATE.PASSWORD_RESET_ERROR,
        LOGIN_STATE.PASSWORD_RESET_LOADING,
      ].includes(state.login_state);
    },
  },
  actions: {
    reset({ commit }) {
    ...
    login({ commit }, { username, password }) {
      ...
      Request.login({ username, password })
        .then(({ updatePassword, token }) => {
          if (updatePassword) {
            commit('login_setState', LOGIN_STATE.PASSWORD_RESET);
          } else {
    ...
    updatePassword({ commit }, { username, password, newPassword }) {
      commit('login_setState', LOGIN_STATE.PASSWORD_RESET_LOADING);
      Request.updatePassword({ username, password, newPassword })
        .then(({ token }) => {
          commit('login_setState', LOGIN_STATE.SUCCESS);
          commit('login_setToken', token);
        })
        .catch(err => {
          commit('login_setState', LOGIN_STATE.PASSWORD_RESET_ERROR);
          return err.message;
        });
    },
  },
